...
/*
Like the previous function this also uses direct pixel access and draws a rectangle for each non-black pixel in the source surface
Except this one cycles through the rainbow colors each time!
*/

void lgbt_draw_text_gay(const char *s,int cx,int cy,int scale)
{
 int x,y,i,c,cx_start=cx;
 int sx,sy,sx2,sy2,dx,dy; /*x,y coordinates for both source and destination*/
 Uint32 pixel/*,r,g,b*/; /*pixel that will be read from*/
